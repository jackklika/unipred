...
pub struct Kalshi {
    ...
    /// - `api_key_id`: A field for storing the API key ID.
    api_key_id: Option<String>,
    /// - `private_key`: A field for storing the RSA private key for signing requests.
    private_key: Option<Arc<RsaPrivateKey>>,
    /// - `client`: The HTTP client used for making requests to the marketplace.
    client: reqwest::Client,
}

impl Clone for Kalshi {
    fn clone(&self) -> Self {
        Kalshi {
...
impl fmt::Debug for Kalshi {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("Kalshi")
            ...
            .field(
                "private_key",
                &self.private_key.as_ref().map(|_| "RsaPrivateKey"),
            )
...
impl Kalshi {
    ...
    /// Checks if the Kalshi instance is configured for API key authentication.
    pub fn has_api_key(&self) -> bool {
        self.api_key_id.is_some() && self.private_key.is_some()
    }

    /// Adds authentication headers to a request builder.
    ///
    /// This function checks if API key authentication is configured. If so, it adds the
    /// necessary signed headers. Otherwise, it falls back to using a token.
    fn add_authentication_to_request(
        &self,
        mut request_builder: RequestBuilder,
        method: &str,
        path: &str,
    ) -> Result<RequestBuilder, KalshiError> {
        if self.has_api_key() {
            let headers = self.get_api_key_headers(method, path)?;
