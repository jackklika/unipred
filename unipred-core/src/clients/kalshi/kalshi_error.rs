...
pub enum KalshiError {
    ...
    /// Error for when an authenticated request is made without logging in.
    NotAuthenticated,
...
impl fmt::Display for KalshiError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            KalshiError::NotAuthenticated => write!(f, "Not authenticated. Please login first."),
            KalshiError::CryptoError(e) => write!(f, "Crypto Error: {}", e),
            KalshiError::IoError(e) => write!(f, "I/O Error: {}", e),
        }
    }
}
...
impl Error for KalshiError {
    fn source(&self) -> Option<&(dyn Error + 'static)> {
        match self {
            ...
            KalshiError::NotAuthenticated => None,
            KalshiError::CryptoError(_) => None,
            KalshiError::IoError(e) => Some(e),
        }
    }
}
...
impl From<rsa::pkcs1::Error> for KalshiError {
    fn from(err: rsa::pkcs1::Error) -> Self {
        KalshiError::CryptoError(err.to_string())
    }
}
...
impl fmt::Display for RequestError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            RequestError::SerializationError(e) => write!(f, "Serialization Error. You connected successfully but there was an error parsing the response: {}", e),
